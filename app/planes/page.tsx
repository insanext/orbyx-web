...
const extraConfig: Record<ExtraKey, ExtraConfig> = {
  ...
  emails_campana: {
    ...
    short: "500 correos campaña adicionales.",
    detail: "por 500 correos",
    unitPrice: 1990,
    price_pack2: 1791,
    price_pack3: 1692,
    unitLabel: "pack",
...
const EMAILS_CAMPANA_PACK_SIZE = 500;
...
function PlanesPageContent() {
  ...
  // resumen de "Total mensual" al cambiar de plan incluya el costo de los
  // add-ons que el tenant ya tiene contratados. La compra/gestión real de
  // add-ons (edición + cobro) vive en components/addons/AddonManager.tsx,
  // montado en el dashboard de Facturación y pago — no aquí.
  ...
  const isStarterSelected = selectedPlan.key === "starter";
  ...
  const currentCampanaWaTotal =
    selectedPlan.includedCampanasWa + campanaWaExtras * CAMPANAS_WA_PACK_SIZE;
  ...
  // Los botones +/- SOLO tocan estado local — solo se usan en la
  // simulación pública (sin tenant_id). Para un tenant existente, la
  // compra/gestión real de add-ons vive en AddonManager (dashboard de
  // Facturación y pago), no aquí.
  ...
  function getDowngradeDetails(current: Plan, target: Plan) {
    ...
    if (current.includedCampanasWa > 0 && target.includedCampanasWa === 0) {
      featuresLost.push("Campañas WhatsApp: ya no incluidas");
    } else if (target.includedCampanasWa < current.includedCampanasWa) {
      featuresLost.push(
        `Campañas WhatsApp: ${current.includedCampanasWa.toLocaleString("es-CL")} → ${target.includedCampanasWa} msgs/mes`
      );
    }
  ...
  function extraSupported(extraKey: ExtraKey) {
    ...
    // WA no disponible durante trial Starter
    if (isTrial && selectedPlanKey === "starter" && extraKey === "wa_confirmacion") {
      return false;
    }
  ...
  function extraUnitLabel(extraKey: ExtraKey, count: number) {
    if (extraKey === "wa_confirmacion" || extraKey === "campanas_wa" || extraKey === "emails_campana" || extraKey === "group_capacity") {
              ...
              <div className="p-4">
                ...
                {hasBillingContext ? (
                  <div className="mt-5">
                    ...
                    <div className="mt-3 rounded-xl border border-cyan-300/15 bg-cyan-400/8 px-4 py-3 text-sm text-cyan-100">
                      {slug ? (
                        <>
                          Los add-ons ahora se gestionan desde{" "}
                          <Link
                            href={`/dashboard/${slug}/billing`}
                            className="font-semibold underline"
                          >
                            Facturación y pago → Add-ons
                          </Link>
                          .
                        </>
                      ) : (
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <p className="flex items-center justify-center gap-2 text-center text-xs text-slate-400">
                    ...
                    {!hasBillingContext && isStarterSelected
