              ...
              {tickets.map(t => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left bg-[#0f1729] border border-blue-900/25 hover:border-blue-700/40 rounded-2xl p-4 transition-colors"
                  >
      ...
      {view === 'new' && (
        <>
          ...
          <div className="bg-[#0f1729] border border-blue-900/25 rounded-2xl p-5 space-y-4">
            <div>
              <label className="text-xs text-blue-300/60 mb-1 block">Asunto</label>
              <input
                value={subject}
            ...
            <div>
              ...
              <input
                ...
                className="w-full text-xs text-blue-300/60"
              />
              {files.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {files.map((f, i) => (
                    <span key={i} className="text-xs bg-blue-900/20 border border-blue-900/30 rounded-lg px-2 py-1 text-blue-300 flex items-center gap-1">
                      ...
                      {f.name}
                      <button onClick={() => setFiles(files.filter((_, idx) => idx !== i))} className="hover:text-red-400 ml-1">
      ...
      {view === 'detail' && selectedTicket && (
        <>
          ...
          <div className="bg-[#0f1729] border border-blue-900/25 rounded-2xl p-5 mb-4">
            <h2 className="text-base font-semibold text-white mb-1">{selectedTicket.subject}</h2>
          ...
          {selectedTicket.status !== 'closed' && (
            <div className="bg-[#0f1729] border border-blue-900/25 rounded-2xl p-4">
              <textarea
                ...
                className="w-full bg-[#0a0f1e] border border-blue-900/30 rounded-xl px-3 py-2 text-sm text-white resize-none focus:outline-none focus:border-blue-500/50 transition-colors mb-2"
              />
              <div className="flex items-center justify-between gap-3">
                <input
                  ...
                  className="text-xs text-blue-300/60 flex-1 min-w-0"
                />
                <button
                  onClick={handleReply}
                  disabled={sendingReply || !replyText.trim()}
                  className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 active:scale-95 text-white text-sm font-medium transition-all disabled:opacity-50 shrink-0"
                >
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {replyFiles.map((f, i) => (
                    <span key={i} className="text-xs bg-blue-900/20 border border-blue-900/30 rounded-lg px-2 py-1 text-blue-300 flex items-center gap-1">
                      <i className="ti ti-photo" style={{ fontSize: 12 }} aria-hidden="true" />
                      {f.name}
                      <button onClick={() => setReplyFiles(replyFiles.filter((_, idx) => idx !== i))} className="hover:text-red-400 ml-1">
                        <i className="ti ti-x" style={{ fontSize: 12 }} aria-hidden="true" />
                      </button>
                    </span>
                  ))}
